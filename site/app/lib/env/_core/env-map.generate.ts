...
async function generateEnvExample() {
  ...
  for (const file of serverEnvFiles) {
    ...
    // Tolerate prettier wrapping (`z\n  .object({`) — match z .object across whitespace/newlines.
    const schemaMatches = content.matchAll(/z\s*\.\s*object\s*\(\s*{([^}]+)}/gs);
    for (const match of schemaMatches) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (commentMatch) {
          const inline = commentMatch[1].match(/^(.*?)\s*\*\/\s*$/);
          if (inline) {
            // Single-line JSDoc: /** … */ on one line — don't scan ahead (that would
            // latch onto the next var's comment and silently drop this var).
            currentComment = inline[1];
          } else {
            // Start of a multi-line JSDoc comment
  ...
  let content = `#
# ⚠️ AUTO-GENERATED .env.example — DO NOT EDIT
#
...
`;
  ...
  // Add summary section
  content += `# 📊 ENVIRONMENT VARIABLES SUMMARY
# ================================
...
`;
  ...
  console.log(`✓ Generated .env.example with ${envVars.size} environment variables`);
}
