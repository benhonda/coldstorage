/* Shared "vault" Mac-app mock — the animated proof-of-safety UI used by the
   app-mock hero (full MacMock window) and the Moments section (just the
   thumbnail grid). Lives here, not in either section, so both stay in sync
   with one implementation. Depends on shared/macos-window.jsx being loaded
   first (MacWindow, MacSidebarHeader). */

const { Badge, Button } = window.ColdstorageDesignSystem_41ebaf;
...
function ThumbGrid({ count, cols, size = "1 / 1", radius = 10, seed = 0, shown = true, stagger = 0 }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: 8 }}>
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} style={{
          aspectRatio: size, borderRadius: radius,
...
function SourceRow({ icon, label, status, tone }) {
  const dotColor = tone === "warn" ? "var(--hue-amber)" : "var(--positive)";
  return (
    <div style={{ display: "grid", gridTemplateColumns: "24px 1fr", gap: 8, alignItems: "start", padding: "7px 10px", margin: "0 10px", borderRadius: 8 }}>
      <span className="csf-icon" style={{ fontSize: 17, color: "rgba(0,0,0,0.5)", marginTop: 1 }}>{icon}</span>
      <div>
        <div style={{ font: "500 12.5px/1.3 var(--font-ui)", color: "rgba(0,0,0,0.85)" }}>{label}</div>
        <div style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 2 }}>
          <span style={{ width: 6, height: 6, borderRadius: 999, background: dotColor, flexShrink: 0 }}></span>
          <span style={{ font: "400 11px/1.3 var(--font-ui)", color: "rgba(0,0,0,0.5)" }}>{status}</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <React.Fragment>
      ...
      <div style={{ padding: "4px 20px 8px" }}>
        <div className="csf-mono" style={{ font: "500 11px/1 var(--font-mono)", color: "rgba(0,0,0,0.6)" }}>148 GB of 500 GB</div>
...
function VaultContent({ phase }) {
  ...
  return (
    <div style={{ padding: "8px 16px 16px", fontFamily: "var(--font-ui)" }}>
      <div style={{
        ...
        {catching ? (
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <Badge tone="accent" icon="cloud_upload">catching up</Badge>
            <span style={{ font: "500 14px/1.3 var(--font-ui)", color: "var(--text-primary)" }}>Archiving newest first — 96 of 148 GB</span>
          </div>
        ) : (
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <Badge tone="success" icon="check">safe</Badge>
            <span style={{ font: "500 14px/1.3 var(--font-ui)", color: "var(--text-primary)" }}>Your last 30 days are safe ✓</span>
          </div>
        )}
      ...
      <div style={{ margin: "16px 2px 8px", font: "600 13px/1 var(--font-ui)", color: "var(--text-secondary)" }}>November</div>
      <ThumbGrid count={14} cols={7} shown={phase !== "idle"} stagger={phase === "run" ? 90 : 0} />
      <div style={{ margin: "16px 2px 8px", font: "600 13px/1 var(--font-ui)", color: "var(--text-secondary)" }}>October</div>
...
function MacMock() {
  ...
  const [phase, setPhase] = React.useState(reduced ? "done" : "idle");
  ...
  React.useLayoutEffect(() => {
    ...
    return () => { window.removeEventListener("resize", measure); clearTimeout(timer.current); };
  }, []);
  ...
  return (
    <div ref={viewRef}>
      <div ref={measureRef} style={{ width: "100%", height: H * fit.scale }}>
        <div style={{ transform: `scale(${fit.scale})`, transformOrigin: "top left", width: W, marginLeft: fit.offset }}>
      ...
      {!reduced && (
        <div style={{ display: "flex", justifyContent: "center", marginTop: 18, opacity: phase === "done" ? 1 : 0, transition: `opacity 300ms ${CS_EASE}`, pointerEvents: phase === "done" ? "auto" : "none" }}>
          <Button variant="ghost" size="sm" icon="replay" onClick={replay}>Replay</Button>
        </div>
      )}
    </div>
  );
...
Object.assign(window, {
  CSC_TINTS, ThumbGrid, SourceRow, VaultSidebar, VaultContent, MacMock,
});
