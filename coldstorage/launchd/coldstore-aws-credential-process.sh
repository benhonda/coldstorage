...
# Installed + Keychain seeded by `task daemon:creds`.
set -euo pipefail

akid=$(security find-generic-password -s coldstorage-aws-access-key-id  -w 2>/dev/null) || {
  echo "coldstore creds: Keychain item 'coldstorage-aws-access-key-id' missing — run 'task daemon:creds'" >&2; exit 1; }
secret=$(security find-generic-password -s coldstorage-aws-secret-access-key -w 2>/dev/null) || {
  echo "coldstore creds: Keychain item 'coldstorage-aws-secret-access-key' missing — run 'task daemon:creds'" >&2; exit 1; }
