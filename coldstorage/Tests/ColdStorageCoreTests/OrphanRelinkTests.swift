...
@Suite struct OrphanRelinkTests {
    ...
    private func fixture() throws -> (engine: UploadEngine, journal: Journal, keys: LocalFileKEK, store: RecordingStore, source: LocalDirSource, base: URL) {
    ...
    @Test func healthyVerifiedBlobIsSilentNoOpOnReRun() async throws {
        ...
        let items = try await f.source.enumerate()
        _ = try await f.engine.run(source: f.source)                  // first pass archives end-to-end
        ...
        _ = try await f.engine.run(source: f.source)                  // second pass: fully linked → nothing to do
