...
@Suite struct StreamingUploadTests {
    ...
    /// Records every part it was handed, so "which bytes actually went over the wire" is inspectable — the
    /// only way to prove a resumed blob SKIPS the parts already on S3 instead of re-sending them.
    final class RecordingStore: BlobStore, @unchecked Sendable {
        private let lock = NSLock()
        private var _created: [String] = []
        private var _parts: [Int: Data] = [:]
        private var _completed = false
        /// Parts S3 claims to already hold — the state a killed run leaves behind.
        var pretendAlreadyOnS3: Set<Int> = []

        var createdKeys: [String] { lock.withLock { _created } }
        var uploadedPartNumbers: [Int] { lock.withLock { _parts.keys.sorted() } }
        var assembled: Data { lock.withLock { _parts.keys.sorted().reduce(Data()) { $0 + _parts[$1]! } } }
        ...
        func existingParts(key: String, uploadId: String) async throws -> Set<Int> { pretendAlreadyOnS3 }
        func uploadPart(key: String, uploadId: String, number: Int, data: Data) async throws -> (etag: String, sha: String) {
            lock.withLock { _parts[number] = data }
            return ("etag-\(number)", "sha-\(number)")
        }
        func complete(key: String, uploadId: String, parts: [PartRow]) async throws { lock.withLock { _completed = true } }
        func verify(key: String) async throws {}
    }

    private struct Fixture {
        let engine: UploadEngine, journal: Journal, keys: LocalFileKEK
        let store: RecordingStore, source: LocalDirSource, root: URL, base: URL
    }

    private func fixture(fileBytes: Data = Data("hello streaming world".utf8)) throws -> Fixture {
    ...
    @Test func archivesByStreamingStraightIntoTheUpload() async throws {
        ...
        let failures = try await f.engine.run(source: f.source)

        #expect(failures.isEmpty)
        #expect(try f.journal.isFileArchived(items[0].id) == true)
        #expect(f.store.completed)
        #expect(f.store.uploadedPartNumbers == [1])            // one small blob → one final part
        #expect(f.store.assembled.count == EnvelopeCipher.encryptedSize(ofPlaintext: items[0].size))
    }
    ...
    @Test func predictedEncryptedSizeMatchesWhatIsActuallyProduced() async throws {
        ...
        _ = try await f.engine.run(source: f.source)

        #expect(f.store.assembled.count == EnvelopeCipher.encryptedSize(ofPlaintext: plaintext.count))
    }
    ...
    @Test func aSourceThatChangedSinceTheScanFailsInsteadOfArchivingSilently() async throws {
        ...
        // Scan first (this is what fixes contentHash + expectedSha256), THEN change the file underneath.
        let items = try await f.source.enumerate()
        let drifted = DriftingSource(items: items, root: f.root)

        let failures = try await f.engine.run(source: drifted)
        ...
        #expect(f.store.completed == false)                              // the torn object was never completed
    }

    /// Hands back items the scan produced, but rewrites the file on disk first — so `open()` streams bytes
    /// that no longer match the `expectedSha256` the plan carries. Exactly the race where a user saves over a
    ...
    /// A photo has no pre-read hash to check against (`expectedSha256` is nil — PhotoKit only produces the
    /// bytes once it streams them), so the guard must stay OUT of its way rather than failing every photo.
    @Test func aSourceWithNoPreReadHashIsArchivedNormally() async throws {
        ...
        let photo = IngestItem(id: "photo-1", relativePath: "IMG_0001.HEIC", size: bytes.count,
                               contentHash: "some-asset-id",      // an identity, NOT a hash of anything
                               expectedSha256: nil,               // …which is exactly why there's nothing to check
                               createdAt: nil, isFavorite: false,
                               open: { AsyncThrowingStream { c in c.yield(bytes); c.finish() } })

        let failures = try await f.engine.run(source: StaticSource(items: [photo]))
    ...
    @Test func archivingALargeFileHoldsAPartInMemoryNotTheBlob() async throws {
        /// Discards parts instead of retaining them — a store that KEPT them would hold the whole blob in
        /// memory itself and make the measurement meaningless (RecordingStore does exactly that, on purpose).
        final class DiscardingStore: BlobStore, @unchecked Sendable {
            func createUpload(key: String) async throws -> String { "u" }
        ...
        let journal = try Journal(path: base.appendingPathComponent("j.sqlite").path)
        let engine = UploadEngine(journal: journal, store: DiscardingStore(),
                                  keys: LocalFileKEK(path: base.appendingPathComponent("kek.bin").path))

        let before = currentRSS()
        _ = try await engine.run(source: LocalDirSource(root: root))
