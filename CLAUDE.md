...
- `account-backend/` — Hono API on Vercel + Neon/Drizzle: Cognito↔Paddle↔zero-knowledge-key-blob backend for the multi-user/paid layer (Taskfile `backend:*`; see [`PROD.md`](./PROD.md) Phase 4). **Scaffolded, not deployed** — needs a real Vercel project + Neon DB + Paddle credentials before it does anything live.
...
- `infra/account-backend/` — Terraform/Terragrunt for the account-backend's Vercel project (OIDC role + TF-managed env vars, Cognito ids read from `infra/coldstorage`; Taskfile `tf:account-backend:*`). **Plans clean vs real AWS/Vercel providers, not yet applied** — blocked on a real `vercel_project_id`.
