...
| render-not-aschild | Base UI composes with a `render` prop, **not** Radix's `asChild`: `<Button render={<a href="/x" />}>Label</Button>`, not `<Button asChild><a href="/x">Label</a></Button>` | every shadcn example online is Radix-based; `asChild` isn't in the props and fails to compile |
| no-fork | don't fork/rewrite generated component source to change how it *looks* — restyle via theme variables. Where you do add behaviour to a generated file (a `loading` prop on `Button`), know that re-running `shadcn add <name>` offers to overwrite it and takes the addition with it — re-apply after any regeneration | components stay default; theming is owned by `references/theming.md`. The CLI treats the file as its output, not yours |
