...
| no-transactions | `neon-http` has **no transaction support** — `db.transaction()` throws `No transactions support in neon-http driver`. For a multi-statement write, order it so any partial failure lands on the safe side (apply revocations/deletes **before** grants/inserts, so an interruption leaves less access than intended, never more) and say so in a comment at the call site | the reflex is a transaction; discovering it doesn't exist halfway through writing one is expensive, and silently shipping a non-atomic mutation is worse |
| no-queries-in-loops | one statement = one HTTP round trip on `neon-http`. Never put a query inside a loop — batch with `inArray(...)` + a single bulk `insert().values([...]).returning()`, then map results by key | a 4-query loop over 14 rows is ~56 sequential HTTPS requests; the same code on a pooled driver would be unremarkable, so the cost is invisible until it isn't |
...
| correlated-counts | never interpolate a column into a raw `sql` template used as a subquery inside `db.select()`; reach for `db.$count(table, filter)` | inside a select drizzle renders interpolated columns **unqualified**, so an outer `projectsTable.id` compiles to a bare `"id"` that Postgres binds to the *inner* table — the correlation is then never true. Valid SQL, no error, every count silently reads 0. `$count` also decodes `count(*)`'s bigint to a number, which a hand-written `sql<number>` only *claims* (the driver returns a string, so `=== 0` fails) |
