...
## Deploy
`task tf:<component>:plan ENV=production` (plans shared then the env) → review → user runs `task tf:<component>:apply ENV=production`. For two envs, repeat with `ENV=staging`. Set manual-secret values in the Vercel dashboard after first apply. Commands live in `references/taskfile.md`.
...
## Verify at latest
- **Terraform, Terragrunt, AWS + Vercel providers** — current versions on the Registry; use `~>`; confirm current schemas (`vercel_project_environment_variable`, `vercel_custom_environment`, `aws_iam_openid_connect_provider`).
